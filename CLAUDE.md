...
- **Cache version ปัจจุบัน**: `po-tracker-v56` — bump ทุกครั้งที่ modify cached files (ตอนนี้ cache: index / po-mobile / po-desktop / **erp** / **stock**)
...
| `config` | Master lists dropdown (config_type: company/car_model/part_name/defect_reason/**stock_unit** — เพิ่ม 2026-09-26 พร้อมขยาย CHECK) | ⚙️ CONFIG |
...
| `stock_items` | วัสดุในคลัง — qty_current, **qty_min** (ห้ามต่ำกว่า), **qty_target** (ค่ากลาง), cost_per_unit (เฉลี่ยเคลื่อนที่), ministry_seq · **`report_group`** (2026-09-25: ชื่อกลุ่มรวมยอด เช่นเฉด 801/802 → 'สีจริง' · NULL = เดี่ยว) | ใหม่ 2026-08-05 |
...
- **`reset_stock_baseline()` = ฟังก์ชันเดียวที่ลบ ledger ได้** — admin เท่านั้น (hardcode `role`, **ไม่มี permission flag ให้เปิดให้ใคร**) · ต้องส่ง `p_confirm='RESET-STOCK'` ไม่งั้น `RAISE EXCEPTION` · client ดาวน์โหลด Excel สำรองให้ก่อนเสมอ ถ้าสำรองไม่สำเร็จจะไม่ล้าง
- **นับสต๊อกทั้งคลัง ใช้ `apply_stock_movement` type `adjust` วนทีละแถว** ไม่ใช่ฟังก์ชันใหม่ — ประวัติเดิมอยู่ครบ และแถวที่ชนกับคนอื่นจะพลาดตัวเดียว ไม่ล้มทั้งชุด
- Storage bucket `stock-docs` เก็บรูปใบส่งของ (ย่อ 1400px/JPEG q0.72 ฝั่ง client)
- **Supabase free tier หลับเองเมื่อไม่มีคนใช้** — **เกิดแล้ว 3 ครั้ง** (2026-07-30 หลังเงียบ 3 เดือน · 2026-08-25 · 2026-09-25 หลังเงียบ 2 สัปดาห์) ถ้าต่อไม่ติดให้เช็ค `status`: `INACTIVE` = หลับ ต้อง `restore_project` แล้วรอ ~30 วิ จน `ACTIVE_HEALTHY`. **ระหว่างหลับ = ทั้ง PO Tracker และคลังสต๊อกใช้ไม่ได้เลยทุกเครื่อง** คนงานจะเห็นหน้าล็อกอินค้างหรือโหลดไม่ขึ้น — ไม่ใช่แค่ปัญหาของ session ที่กำลังทำงาน. **เช็คทุกครั้งที่เปิด session ใหม่ก่อนจะสรุปว่าอะไรพัง** และถ้าเริ่มใช้จริงทุกวันแล้วปัญหานี้จะหายไปเอง (มีคนเข้าทุกวัน = ไม่หลับ)
...
| BUG46 (stock, ตัวเอง — **production ล่ม ~20 นาที**): กดรับเข้า/เบิกออกไม่ได้เลยทั้งระบบ ขึ้น `Could not choose the best candidate function` | ✅ Fixed (2026-08-14) migration `drop_stale_apply_stock_movement_overload` → ตอนเพิ่ม `p_created_at` (บันทึกย้อนหลัง) ผมใช้ `CREATE OR REPLACE` **แต่ `CREATE OR REPLACE` แทนที่เฉพาะฟังก์ชันที่ signature ตรงเป๊ะเท่านั้น** เพิ่มพารามิเตอร์ = สร้างฟังก์ชันตัวใหม่ → มี 2 ตัวซ้อน ทั้งคู่รับ 10 named args ที่ปุ่มส่งมา (ตัวที่ 11 มี default) → PostgREST เลือกไม่ได้ ตอบ **PGRST203 ทุกครั้ง**. **ซ้ำรอยกับดักที่ migration เมื่อวานเพิ่งลบ overload 7 args ทิ้งเพื่อเลี่ยง — และคอมเมนต์ตรงนั้นเขียนคำว่า PGRST203 ไว้ด้วยซ้ำ** เขียนเตือนไว้เฉย ๆ ไม่พอ เพราะไม่มีอะไรตรวจ. **ทำไม test จับไม่ได้:** ชุดทดสอบขับ UI ผ่าน shim ที่ resolve `rpc()` ด้วยชื่อฟังก์ชันอย่างเดียว — ปัญหา overload มีเฉพาะใน PostgREST จริง เทสต์บนเบราว์เซอร์ไม่มีทางเห็น. **กฎใหม่: ทุกครั้งที่แก้พารามิเตอร์ของ DB function ต้อง `select oid::regprocedure from pg_proc where proname='<fn>'` แล้วยืนยันว่าเหลือแถวเดียว · เพิ่มพารามิเตอร์ = DROP แล้ว CREATE ห้ามใช้ CREATE OR REPLACE** |
| BUG47 (stock, มือถือ): ช่องวันที่ "เลือกช่วง" ในแท็บรายวันเป็น input ดิบของเบราว์เซอร์ สูงแค่ 21px + ตัดบรรทัดเกะกะบนจอ 320px | ✅ Fixed (2026-09-25) → style ของ `.period-bar` อยู่**ใน media query ของ PC เท่านั้น** มือถือเลยไม่ได้อะไรเลย. ย้ายออกมาใช้ทุกขนาดจอ + รองรับ `input[type=date]` + `min-height: var(--touch-min)` + จอ ≤430px วางเป็น grid 2 แถว. **เจอจาก audit อัตโนมัติ 3 ขนาดจอ** (320/360/390) ที่ไล่วัดขนาดทุกปุ่ม |
| BUG48 (stock, มือถือ): ชื่อแท็บตัดกลางคำ "ผู้ / ดูแล", "คลัง / ปัจจุบัน" 3 บรรทัด บนจอ 320-360px | ✅ Fixed (2026-09-25) → 4 แท็บเรียงแถวเดียวไม่พอที่ ข้อความเลยพับตามใจเบราว์เซอร์ซึ่งตัดภาษาไทยผิดที่. แยก emoji/ข้อความเป็น `.vs-ic`/`.vs-lb` · ข้อความ `nowrap` · ≤430px วาง icon บน ข้อความล่าง (วัดแล้ว "คลังปัจจุบัน" กว้าง 59px ในปุ่ม 68px ที่ 320px) · เพิ่ม `white-space: nowrap` ให้ `.mv-po` ด้วย (เลข PO ในประวัติหักที่ขีด "PO-/23/04/69-01") |
| BUG49 (stock): ปุ่ม "ยกเลิก" โผล่บนแถว **ยอดตั้งต้น** → กดแล้วยอดวัสดุนั้นกลายเป็น 0 | ✅ Fixed (2026-09-25) → `reverse_stock_movement` ไม่ได้กันแถว `initial` (อ่านผลจาก balance chain แล้วกลับด้านเฉย ๆ). ซ่อนปุ่มฝั่ง client (`canUndo && movement_type !== 'initial'`) — ยอดตั้งต้นผิดให้ใช้ **ปรับยอด** ที่เก็บประวัติ. ไม่ได้แก้ server เพราะไม่ใช่เรื่องสิทธิ์ (คนกดได้คือคนมี `stockReverse` อยู่แล้ว) แค่กันกดพลาด — **สำคัญหลังตั้งต้นใหม่ เพราะทุกวัสดุจะมีแถว initial ที่มีปุ่มนี้** |
| BUG38 (stock + erp + mobile): **toast เป็นแผ่นใสบังปุ่มถาวร** — ปุ่มบางปุ่มกดไม่ติดหลังมี toast ขึ้นครั้งแรก | ✅ Fixed (2026-08-13) → `#toast` ไม่มี `pointer-events: none` และ `toast()` ไม่เคยล้างข้อความทิ้ง → หลัง toast แรก div ยังกว้างเท่าข้อความเดิม ลอยอยู่ `z-index:9999` ที่ `bottom:40px` กลางจอ **ตลอดเวลา** ทั้งที่ `opacity:0` มองไม่เห็น → ทับปุ่มยืนยัน/บันทึกที่อยู่ท้าย modal พอดี. **จับได้จากการทดสอบบนจอมือถือ** (Playwright รายงาน `<div id="toast">รับเข้าเรียบร้อย</div> intercepts pointer events`) ไม่ใช่จากอ่านโค้ด. แก้ 1 บรรทัดใน `stock.html` + `erp.html` + `po-mobile.html` (po-desktop ใช้ `#toast-container` ที่มี `pointer-events:none` อยู่แล้ว). **น่าจะเป็นต้นเหตุเดียวกับอาการ "ปุ่มเหมือนกดไม่ติด" ที่เจอซ้ำ ๆ** |
| BUG39 (stock): **ยกเลิกรายการถูกนับเป็นการซื้อเข้า** — รายงานรายเดือน + รายงานกระทรวงบวมทั้งสองฝั่ง | ✅ Fixed (2026-08-13) → รายการยกเลิกถูกเขียนเป็น movement ตรงข้าม (ยกเลิกการเบิก = แถว `in`) ส่วน `buildMonthlySeries` แบ่งกระเป๋าจาก `movement_type` อย่างเดียว → "ยกเลิกการเบิก 40 กก." กลายเป็น "ซื้อเข้า 40 กก." **ซ้ำร้าย `loadAllMovements` ไม่ได้ select `id` กับ `reverses_id` มาด้วยซ้ำ** โค้ดจึงมองไม่เห็นว่าแถวไหนเป็นการยกเลิก. ผลจากการทดสอบ: เดือน ส.ค. รายงาน "ซื้อ 11,397 บาท" ทั้งที่ซื้อจริง 8,375 (**เกินจริง 27%**) · รายงานกระทรวงทินเนอร์ขึ้น รับ 210/ใช้ 140 ทั้งที่จริง 150/80 · ราคาต่อหน่วยที่รายงานก็เพี้ยนตาม (54.27 แทน 55.83). **สมการ ยอดยกมา+รับ−ใช้=คงเหลือ ยังบาลานซ์** ฟอร์มกระทรวงจึงผ่าน validation ทั้งที่ตัวเลขไม่จริง — จับด้วยตาไม่ได้เลย. แก้: select `id,reverses_id` เพิ่ม + `bucketTypeOf()` ชี้แถวยกเลิกกลับไปลงกระเป๋าของแถวต้นฉบับ (delta จาก balance chain มีเครื่องหมายอยู่แล้ว เลยกลายเป็นลบให้เอง) |
| BUG40 (stock): **มูลค่าคงคลังของเดือนที่ผ่านไปแล้วเปลี่ยนตามราคาวันนี้** | ✅ Fixed (2026-08-13) → `closingValue` คูณด้วย `stock_items.cost_per_unit` **ปัจจุบัน** ทุกเดือน → ซื้อของแพงวันนี้ มูลค่าปลายเดือน มิ.ย. ขยับตาม (ทดสอบ: มิ.ย. ควรเป็น 29,300 แต่ขึ้น 29,981 หลังซื้อล็อตใหม่). **เป็นข้อบกพร่องเดียวกับที่ decision log บอกว่าแก้ไปแล้ว** — คราวนั้นแก้แค่ค่า in/out (ตีตรา `unit_cost` ลงแถว) แต่ **ลืมยอดคงคลังปลายเดือน**. แก้: replay ค่าเฉลี่ยเคลื่อนที่จาก ledger ตามสูตรเดียวกับ server (`initial` ตั้งค่า · `in` ผสมใหม่ · `out`/`adjust`/แถวยกเลิก ไม่แตะ) เก็บเป็น `closingAvgByItem[item][month]` |
| BUG41 (stock): เบิกเกินยอดได้ไม่จำกัด → ยอดติดลบ + **ค่าเฉลี่ยราคาถูกล้างทิ้ง** | ✅ Fixed (2026-08-13) DB migration `block_negative_stock_balance` → ทดสอบเบิก 9,999 จากของที่มี 140 ผ่านฉลุย เหลือ −9,859. ที่อันตรายกว่าคือ **รับเข้าตอนยอดติดลบทำให้ราคาเฉลี่ยหาย** — สูตรผสมด้วย `GREATEST(qty_current,0)` = 0 → ยอด −300 @62.50 รับเข้า 100 @40 ได้เฉลี่ย **40 ถ้วน** ต้นทุนของเก่าหายเกลี้ยง. แถมรายงานกระทรวงพิมพ์ยอดคงเหลือติดลบ = ส่งไม่ได้. แก้: server `RAISE EXCEPTION 'INSUFFICIENT_STOCK:<ยอดที่มี>'` + client แจ้ง "เบิกได้ไม่เกิน N — ให้บันทึกรับเข้า หรือปรับยอดให้ตรงก่อน" (เดิมเป็น `confirm()` ที่กดผ่านได้). ปลอดภัยที่จะบล็อก เพราะ ledger ยังไม่เคยถูกใช้จริง |
| BUG42 (stock): การ์ด "เบิกใช้รอบนี้" คิดจากราคาเฉลี่ยวันนี้ ไม่ใช่ราคาที่ตีตราไว้ | ✅ Fixed (2026-08-13) → `renderStockSummary` คูณ `monthAgg.out × cost_per_unit` ปัจจุบัน ทั้งที่ `buildMonthlySeries` คำนวณ `outValue` ที่ตีตราไว้ให้แล้ว แต่ `deriveMonthAgg` ทิ้งไป เหลือแต่จำนวน. แก้: ส่ง `inValue`/`outValue` ผ่าน `monthAgg` แล้วใช้ค่านั้น. ตัวเลขเพี้ยน 104,653 vs 104,437 ในการทดสอบ |
| BUG43 (stock): `loadAllMovements` ไม่มี `.limit()` → PostgREST ตัดที่ 1,000 แถวเงียบ ๆ | ✅ Fixed (2026-08-13) → เรียง `created_at` ขึ้น + ไม่มี limit = พอ ledger โต 1,000 แถว **รายเดือนจะค้างอยู่ที่ข้อมูลเก่า ไม่อัปเดตอีกเลย** โดยไม่มี error. ยังไม่เกิดเพราะมี 15 แถว แต่ระเบิดเวลาแน่นอนเมื่อเริ่มใช้จริง. แก้: `.limit(20000)` + `console.warn` เมื่อชนเพดาน |
| BUG44 (stock, มือถือ): ปุ่ม "ยืนยัน" ของป๊อปอัพรับเข้า/เบิกออก **อยู่ใต้ขอบจอ** บนมือถือจอสั้น | ✅ Fixed (2026-08-13) → `.modal-sheet` สูงได้ถึง 90vh แล้วเลื่อนข้างใน — ฟอร์มรับเข้ามี 7 ช่อง (จำนวน/ราคา/ใบส่งของ/ผู้ขาย/รูป/หมายเหตุ) ยาวเกินจอ **iPhone SE (568px) และ iPhone 14 (664px) เปิดมาไม่เห็นปุ่มยืนยัน ต้องเลื่อนหาเอง** (Pixel 7 จอ 839px ไม่เจอ เลยรอบแรกไม่เห็น). แก้: `position: sticky; bottom: 0` ตรึงปุ่มไว้ท้ายกล่อง ช่องกรอกเลื่อนผ่านข้างหลัง + `box-shadow` ทาสีพื้นกันเนื้อหาโผล่ใต้ปุ่ม. **ทดสอบ 3 ขนาดจอจริง (320/390/412px) ผ่านหมด** |
| BUG45 (stock): **ยอดตั้งต้นถูกนับเป็น "ซื้อเข้า"** ในกราฟรายเดือน + รายงานกระทรวง | ✅ Fixed (2026-08-13) → `initial` ตกลงกระเป๋าเดียวกับ `in` → เดือนที่เปิดใช้ระบบจะรายงานว่า**ซื้อวัตถุดิบ 135,407 บาท ทั้งที่ไม่ได้ซื้ออะไรเลย** เป็นแค่ของที่มีอยู่ในโกดังอยู่แล้ว. รายงานกระทรวงก็เล่าเรื่องเดียวกัน (ยอดยกมา 0 · รับ 135,407) ซึ่งถ้าเทียบกับใบกำกับภาษีจะไม่ตรง. **เห็นจากการดูกราฟบนจอมือถือ** — แท่งเดือนแรกสูงลิ่วผิดสัดส่วน. แก้: แยก `initQty` ออกมาไม่นับทั้ง purchase และ consume → สมการ `ยอดยกมา = คงเหลือ − รับ + ใช้ − ปรับยอด` derive ยอดตั้งต้นเป็น **ยอดยกมา** ให้เอง ซึ่งตรงความจริง |
...
**⏸️ ค้างอยู่ 2 เรื่อง:**
1. ~~**นับสต๊อก (stocktake)**~~ ✅ **เสร็จแล้ว 2026-09-09** — โหมด "📋 นับสต๊อก" + "🧹 ตั้งต้นใหม่" ในแท็บ 🔐 ผู้ดูแล (admin เท่านั้น) ดูวิธีใช้ที่หัวข้อ "ลำดับการเริ่มใช้สต๊อกจริง" ด้านล่าง
...
**✅ คำถามเรื่องสี — ตอบแล้ว (2026-08-13):** โรงงาน**ผสมสีเอง** คนงานผสมมือตามสูตรโปรแกรม Nippon Paint · มี**เครื่องวัดสี**แล้ว + ดูแผ่นพ่นด้วยสายตาควบคู่ · โปรแกรม **export ได้** · **เครื่องชั่งไม่ต่อคอม**
→ toun สั่ง**พักไว้** ทำเรื่องสต๊อกให้จบก่อน · บริบทครบทั้งหมดอยู่ที่ **`docs/BACKLOG-COLOR-MIXING.md`**

**⚠️ ผลตรวจสต๊อก (2026-08-13):** โมดูลเสร็จครบแต่**ยังไม่มีใครใช้เลย** — รายการเคลื่อนไหว 15 แถวเป็นยอดตั้งต้นล้วน (2026-07-25) รับเข้า/เบิกออก/ปรับยอด = 0 ทั้งหมด · สาเหตุหลัก: `stockIn/Out/Adjust/Reverse` เป็น true เฉพาะ admin ผู้ใช้อีก 9 คนกดบันทึกไม่ได้ · `qty_target` ว่าง 15/15 → ป้าย "ควรสั่ง N" ไม่เคยทำงาน · ต่ำกว่าขั้นต่ำแล้ว 3 รายการ (ทินเนอร์ 80/250 · ทรายขัดน้ำ 140/290 · สีโป๊วแดง 15/18) — รายละเอียดใน `docs/ERP-STOCK.md` §8

---

### 📋 ลำดับการเริ่มใช้สต๊อกจริง (toun ถาม 2026-09-25 — ยังไม่ได้ลงมือ)

**⚠️ กับดักที่ต้องกันไว้: "ตั้งต้นใหม่" ไม่ใช่ปุ่มล้างเป็นศูนย์แล้วค่อยกลับมากรอกทีหลัง** — หน้าจอนั้น**ขอยอดที่นับได้จริงตั้งแต่ตอนกด** toun เข้าใจตอนแรกว่าต้อง "ล้างก่อน → แล้วไปเดินนับ → แล้วเอามาใส่ช่องนับสต๊อก" ซึ่งจะได้รายการ "ปรับยอด" ที่ไม่จำเป็นตั้งแต่วันแรก. **ลำดับที่ถูกคือนับก่อน แล้วค่อยล้าง**

| ขั้น | ทำอะไร | หน้าไหน |
|---|---|---|
| 1 | เดินนับของจริงทั้งโกดัง จดใส่กระดาษ — **จดราคาต่อหน่วยล่าสุดจากใบส่งของมาด้วย** (ราคานี้คือจุดเริ่มของต้นทุนเฉลี่ย ถ้าใส่มั่วมูลค่าคลัง + รายงานกระทรวงเพี้ยนตามทั้งสาย) | กระดาษ |
| 2 | วัสดุที่ใช้จริงแต่ยังไม่มีในระบบ → กด **"เพิ่มวัสดุ"** ให้ครบก่อน เพื่อให้หน้าตั้งต้นลิสต์ครบในรอบเดียว | 📦 คลังปัจจุบัน |
| 3 | **🧹 ตั้งต้นใหม่** — กรอกยอดนับได้ + ราคา ทุกตัว · กด 🗑 ลบของทดลอง · พิมพ์ `ล้างข้อมูล` · กด · **เซฟไฟล์ Excel สำรองที่เด้งมา** | 🔐 ผู้ดูแล |
| 4 | ใช้งานปกติ — รับเข้า/เบิกออก ทุกวัน | 📦 คลังปัจจุบัน |
| 5 | **สิ้นเดือน** นับอีกรอบ → **📋 นับสต๊อก** ปรับให้ตรง (ประวัติเดิมอยู่ครบ) | 🔐 ผู้ดูแล |

**เทียบให้เห็นภาพ: ตั้งต้นใหม่ = ตั้งเลขศูนย์นาฬิกา (ทำครั้งเดียว) · นับสต๊อก = ตั้งเวลาให้ตรงทุกเดือน (ทำตลอดไป)** — ตอนเริ่มไม่ต้องใช้ "นับสต๊อก" เลย เพราะตั้งต้นใหม่รับตัวเลขที่นับได้ไปแล้วในตัว

**สถานะจริง ณ 2026-09-25 (ก่อนตั้งต้น):**
- วัสดุใช้งาน **16 รายการ** = ของจริงตามรายงานกระทรวง 15 + ของทดลองเหลือ **"สีขาว" ตัวเดียว** (toun ปิด `is_active` ของทดลองไปแล้ว 5 ตัว) → ตอนตั้งต้นกด 🗑 แค่ "สีขาว"
- ประวัติ **19 แถว** รอถูกล้าง (มีของทดลองปน เช่น เบิกสีพื้น 111 ลิตรที่ไม่เคยเกิด · รับเข้าที่เลขใบส่งของเป็น "2")
- มูลค่าที่ระบบบอกตอนนี้ **129,228.80 บาท** — ยังเชื่อไม่ได้จนกว่าจะตั้งต้น
- **`qty_target` ยังว่าง 13/16** → ป้าย "ควรสั่ง N" ยังไม่ทำงานเกือบทั้งคลัง · ต่ำกว่าขั้นต่ำ 3 รายการ
- **ยังไม่มีการใช้งานจริงเลย** — `stock_in`/`stock_out` ล่าสุดคือ 2026-08-25 (ที่เป็นการทดลอง) · ยังไม่เคยมี `stock_stocktake` หรือ `stock_baseline_reset`
...
### 🔨 กำลังทำอยู่ตอนนี้ (As of 2026-09-25)

**ไม่มีงาน code ค้าง** — งานที่ค้างอยู่เป็น**งานของ toun ในโรงงาน** ไม่ใช่งานเขียนโปรแกรม:

| ค้างอยู่ | ใคร | ทำไมสำคัญ |
|---|---|---|
| **เดินนับสต๊อกจริง แล้วกด "ตั้งต้นใหม่"** | toun | ตัวเลขในระบบยังเป็น mockup ทั้งหมด ใช้อ้างอิงอะไรไม่ได้จนกว่าจะทำ |
| ~~**ตั้งรหัส 6 หลักให้พนักงาน 5 คน**~~ | ✅ **toun ทำแล้ว 2026-09-25** | ตรวจฐานแล้ว 10/10 คนเป็น 6 หลัก ไม่มีรหัสซ้ำ |
| **ใส่ `qty_target`** | toun | ว่าง 13/16 → ป้าย "ควรสั่ง N" ไม่ทำงาน |
| **ให้พนักงาน (staff) เปิดหน้าสต๊อกได้** | ✅ **เสร็จ 2026-09-25** — ธงใหม่ `viewStock` · staff/manager เข้าตรงจากปุ่มในหน้า PO · ERP เหลือ admin/supervisor/office · ✅ ซ่อนตัวเลขเงินจาก staff/manager แล้ว (`viewStockDocs`) |

**ก่อนตอบคำถามเรื่อง "ระบบพัง/เข้าไม่ได้" ทุกครั้ง — เช็ค Supabase `status` ก่อน** หลับไปแล้ว 3 ครั้ง

### 🗄️ Deploy state (As of 2026-04-24 end of day)
...
- **ทำไมรายการ "ยกเลิก" ต้องลงกระเป๋าของแถวต้นฉบับ ไม่ใช่กระเป๋าตรงข้าม**: ledger เขียนการยกเลิกเป็น movement ตรงข้าม (ยกเลิกการเบิก = แถว `in`) เพราะยอดคงเหลือต้องถูกต้อง — แต่**ความหมายทางบัญชีไม่ใช่การซื้อ** ถ้าแบ่งกระเป๋าตาม `movement_type` เดือนนั้นจะได้ทั้ง "ซื้อเพิ่ม" และ "ใช้เพิ่ม" ที่ไม่เคยเกิดขึ้น (BUG39 — เกินจริง 27% ในการทดสอบ). ตัวชี้ขาดคือ `reverses_id` → มองหาแถวต้นฉบับแล้วใช้ประเภทของมัน ส่วน delta ที่ derive จาก balance chain มีเครื่องหมายกลับด้านให้อยู่แล้ว **จึงกลายเป็นการลบออกโดยไม่ต้องเขียนเงื่อนไขเพิ่ม**. ผลข้างเคียงที่ถูกต้อง: ยกเลิกข้ามเดือน → เดือนที่ยกเลิกขึ้นยอดติดลบ (ไม่ไปแก้เดือนที่ปิดไปแล้ว) ซึ่งเป็นเคสที่ BUG35 เตรียม `signedIn/signedOut` ไว้รองรับพอดี. **บทเรียนทั่วไป: ถ้าแถวหนึ่งมีทั้ง "รูปแบบทางเทคนิค" และ "ความหมายทางธุรกิจ" อย่าจัดกลุ่มด้วยรูปแบบ**
- **ทำไมมูลค่าคงคลังปลายเดือนต้อง replay ค่าเฉลี่ยจาก ledger ไม่ใช่อ่าน `cost_per_unit`**: `cost_per_unit` คือค่า ณ วันนี้ ช่องเดียว แก้ทับตลอด — เอาไปคูณยอดของเดือน มิ.ย. เท่ากับพูดว่า "ของเมื่อ 2 เดือนก่อนราคาเท่ากับที่เพิ่งซื้อวันนี้" (BUG40). ที่เจ็บคือ**เคยแก้เรื่องนี้ไปแล้วครึ่งหนึ่ง** — ตอนที่ตีตรา `unit_cost` ลงทุกแถวเพื่อกันมูลค่าซื้อ/ใช้ย้อนหลังเปลี่ยน แต่ลืมยอดคงคลัง. ค่าเฉลี่ยย้อนหลัง derive ได้จาก `unit_cost` + `balance_after` ที่มีอยู่แล้ว โดยเล่นสูตรเดียวกับ `apply_stock_movement` ซ้ำ (`initial` ตั้งค่า · `in` ผสมใหม่ · `out`/`adjust`/แถวยกเลิก ไม่แตะ) **ไม่ต้องเพิ่มคอลัมน์**. บทเรียน: เวลาแก้ปัญหา "ประวัติเปลี่ยนได้" ต้องไล่ให้ครบทุกตัวเลขที่อ่านจากช่องปัจจุบัน ไม่ใช่แค่ตัวที่เพิ่งเจอ — `grep cost_per_unit` ทุกจุดที่คูณกับยอดของอดีต
- **ทำไมเลือกบล็อกยอดติดลบที่ server แทนเตือนแล้วให้ผ่าน**: `confirm()` เดิมกดผ่านได้ในวินาทีเดียว และผลที่ตามมามองไม่เห็นจากหน้าจอ — มูลค่าคงคลังติดลบ · **ค่าเฉลี่ยราคาถูกล้างทิ้งตอนรับเข้าครั้งถัดไป** (ผสมด้วย `GREATEST(qty,0)` = 0) · ฟอร์มกระทรวงมียอดติดลบจนส่งไม่ได้. ยอดติดลบไม่เคยเป็นความจริงทางกายภาพ มันคือความผิดพลาดทางบัญชีเสมอ — และทางออกที่ถูกต้องทั้งสองทาง (บันทึกรับเข้าที่ตกหล่น / ปรับยอดให้ตรง) อยู่ห่างแค่ปุ่มเดียว **การปฏิเสธจึงผลักให้ ledger เข้าใกล้ความจริง ไม่ใช่ผลักคนออกจากระบบ**. ข้อความ error พก**ยอดที่มีจริง**ติดไปด้วย เพื่อให้จอที่ค้าง (คนอื่นเบิกตัดหน้า) ยังบอกตัวเลขที่ใช้ได้. ตัดสินใจบล็อกได้เต็มที่เพราะ ledger ยังไม่เคยถูกใช้จริง — ถ้ามีข้อมูลจริงแล้วต้องคิดหนักกว่านี้
...
- **~~ทำไม PIN 4 หลักที่ผิด ต้องรอ 1.5 วิก่อนแจ้งว่าผิด~~ (เลิกใช้แล้ว 2026-09-09 — ดูข้อ "6 หลักเท่านั้น" ท้ายรายการ)**: ระหว่างเปลี่ยนผ่าน admin ใช้ 6 หลัก คนอื่นยัง 4 หลัก แป้นจึงต้องลองที่ 4 ก่อน **แล้วห้ามฟ้องทันที** เพราะหลักที่ 5 อาจกำลังจะมา. แต่ถ้าเงียบไปเลย คนที่รหัส 4 หลักผิดจริงจะเห็นว่า**ไม่มีอะไรเกิดขึ้น** แล้วตัวเลขที่ค้างอยู่จะไปปนกับที่พิมพ์ครั้งถัดไปจนเพี้ยน — **เจอจากการทดสอบ ไม่ใช่จากอ่านโค้ด**. ทางออก: พลาดที่ 4 → ตั้งเวลา 1.5 วิ ถ้ามีหลักที่ 5 มาก็ยกเลิกเวลา ถ้าไม่มีก็ฟ้องว่าผิดแล้วล้างให้
- **~~ทำไมห้ามตั้ง PIN ที่เป็นคำนำหน้าของคนอื่น~~ (เลิกใช้แล้ว 2026-09-09 — ดูข้อ "6 หลักเท่านั้น" ท้ายรายการ)**: พอมีทั้ง 4 และ 6 หลักในระบบเดียวกัน แป้นจะลองที่หลักที่ 4 เสมอ → ถ้าใครถือ `1112` ขณะที่ admin ถือ `111222` **การพิมพ์รหัส admin จะล็อกอินเป็นคนนั้นตั้งแต่หลักที่ 4** โดยไม่มีใครรู้ตัว. `savePin` จึงปฏิเสธรหัสที่ขึ้นต้นเหมือนกันทั้งสองทิศ ไม่ใช่แค่รหัสซ้ำตรง ๆ — **นี่คือสิ่งที่ทำให้ระบบผสม 4/6 หลักปลอดภัยพอจะใช้ได้จริง**
- **ทำไม session ใช้ `sessionStorage` แล้วต่อกันได้ข้ามหน้า**: ทั้ง 4 ไฟล์อยู่ origin เดียวกัน `sessionStorage` แชร์กันภายในแท็บเดียว → ล็อกอินที่ ERP แล้วกดไป PO ในแท็บเดิม = ไม่ต้องใส่ซ้ำ. ที่ผ่านมา PC ถามซ้ำเพราะ **`po-desktop.html` เป็นไฟล์เดียวใน 4 ไฟล์ที่ `setItem` แต่ไม่เคย `getItem` กลับ** — เขียนแล้วไม่เคยอ่าน. เพิ่ม session recovery ให้เหมือนอีก 3 ไฟล์ก็จบ ไม่ต้องเปลี่ยนกลไกอะไรเลย และ timeout 30 นาทีเดิมยังเป็นตัวตัดเหมือนเดิม
- **ทำไม `CREATE OR REPLACE` ใช้ไม่ได้เมื่อเพิ่มพารามิเตอร์ให้ DB function**: Postgres ผูก identity ของฟังก์ชันไว้กับ **signature (ชนิดพารามิเตอร์ครบชุด)** ไม่ใช่ชื่อ → เติมพารามิเตอร์ตัวที่ 11 แล้วสั่ง REPLACE = ได้ฟังก์ชัน**ตัวที่สอง** ไม่ใช่การแทนที่. ถ้าตัวใหม่มี default ทั้งสองตัวจะรับ call เดิมได้เท่ากัน PostgREST เลือกไม่ถูก ตอบ PGRST203 → **ทุกการเรียกล้มหมด ไม่ใช่แค่ของใหม่** (BUG46 ทำให้บันทึกสต๊อกไม่ได้ทั้งระบบ). สิ่งที่ต้องทำคือ `DROP FUNCTION <ลายเซ็นเก่า>` เสมอ แล้วตรวจด้วย `pg_proc` ว่าเหลือแถวเดียว. **บทเรียนที่ลึกกว่านั้น: ผมเขียนคำเตือนเรื่อง PGRST203 ไว้ในคอมเมนต์ migration เมื่อวาน แล้ววันรุ่งขึ้นก็เดินเข้ากับดักเดิม — คอมเมนต์เตือนคนอ่าน แต่ไม่มีอะไรเตือนคนเขียน ต้องมีขั้นตอน "ตรวจ pg_proc หลัง migrate" อยู่ในลิสต์ ไม่ใช่อยู่ในความจำ**
- **ทำไม shim ทดสอบ UI จับบั๊กประเภทนี้ไม่ได้เลย**: shim resolve `sb.rpc('ชื่อ')` ด้วยชื่ออย่างเดียว ส่วน overload resolution เกิดที่ PostgREST/Postgres เท่านั้น → เทสต์ 43/43 ผ่านหมดทั้งที่ production เรียกไม่ได้สักครั้ง. **ชั้นที่ shim จำลองไม่ได้ ต้องตรวจกับฐานข้อมูลจริง** — เหมือนที่เคยสรุปไว้ตอนทดสอบ RPC ว่า shim สะท้อนแค่ความเข้าใจของผม ไม่ใช่ของจริง
- **ทำไมใช้ .xlsx จริง ไม่ใช่ CSV ทั้งที่ CSV ง่ายกว่ามาก**: คนที่จะกรอกคือ**พนักงานสต๊อกที่ใช้โปรแกรมไม่เป็น** — Excel บนเครื่องภาษาไทยเซฟ CSV เป็น codepage ท้องถิ่นโดยดีฟอลต์ **ภาษาไทยกลายเป็นขยะทันทีที่เซฟ** และยังขึ้นกล่อง "จะเก็บรูปแบบนี้ไหม" ที่ทำให้คนไม่กล้ากดต่อ · .xlsx ไม่มีคำถามเรื่อง encoding เลย และเก็บวันที่เป็นวันที่จริงไม่ใช่ข้อความที่ Excel จัดรูปแบบใหม่เอง. ค่าที่จ่ายคือไลบรารี 900KB — แก้ด้วยการ**โหลดตอนกดปุ่มเท่านั้น** คนที่แค่มาเบิกทินเนอร์ไม่ต้องโหลด
- **ทำไมต้องมีหน้า "ตรวจก่อนนำเข้า" ไม่ import ทันที**: ไฟล์ 40 แถวที่พังแถวที่ 23 แล้ว import ไปครึ่งหนึ่ง = สต๊อกอยู่ในสภาพที่ไม่มีใครรู้ว่าจริงหรือไม่จริง และแก้ยากกว่าตอนยังไม่เริ่ม. ตรวจทั้งไฟล์ก่อน → บอกทีละแถวว่าผิดตรงไหน (ไม่มีวัสดุชื่อนี้ / ประเภทผิด / วันที่อนาคต) → คนแก้ในไฟล์แล้วส่งใหม่. **บรรทัดที่เขียนไว้ชัดว่า "ยังไม่มีอะไรถูกบันทึกจนกว่าจะกดปุ่ม" สำคัญพอ ๆ กับตัวตรวจ** เพราะคนที่ไม่มั่นใจจะไม่กล้าลอง
- **ทำไม import ต้องเรียงเก่าไปใหม่ก่อนส่ง**: แต่ละแถวถูกตรวจยอดคงเหลือตอนที่ server ประมวลผล ถ้าส่งสลับลำดับ การเบิกอาจถูกปฏิเสธเพราะของยังไม่ถูกรับเข้าในรอบเดียวกัน. เรียงตามวัน-เวลาก่อน = แถวหลังยืนบนยอดที่แถวก่อนทิ้งไว้ ตรงกับลำดับที่เกิดขึ้นจริง
- **ทำไม backdating ต้องขยับ `balance_after` ของแถวที่อยู่หลังด้วย**: ยอดคงเหลือเป็นค่าสะสม และ**ทั้งรายวันและรายเดือน derive จากผลต่างของ `balance_after` ที่ติดกัน** แทรกแถวกลางโดยไม่ขยับแถวหลัง = chain ขาด รายงานเพี้ยนทั้งหมดโดยไม่มี error. `apply_stock_movement` จึงคำนวณยอดของแถวย้อนหลังจากแถวที่อยู่ก่อนหน้า**ในเชิงเวลา** แล้ว `UPDATE ... SET balance_after = balance_after + delta` ทุกแถวที่อยู่หลัง — ทดสอบกับฐานข้อมูลจริงแล้วได้ chain เดียวกับกรอกตามลำดับเป๊ะ. **สิ่งที่ยอมแลก**: ค่าเฉลี่ยราคายังผสมที่ยอดวันนี้ ไม่ได้ replay จากจุดแทรก (un-blend จากแถวเดียวทำไม่ได้ — `reverse_stock_movement` ก็ยอมแลกแบบเดียวกัน) ส่วนจำนวนและราคาที่ตีตราไว้รายแถว ซึ่งเป็นสิ่งที่รายงานอ่านจริง ยังตรงเป๊ะ
- **ทำไมกัน `RETRO_WOULD_GO_NEGATIVE` แยกจาก `INSUFFICIENT_STOCK`**: แถวย้อนหลังอาจ "ของพอ ณ วันนั้น" แต่ทำให้**วันถัดมาติดลบ** (ทดสอบ: เบิก 90 วันที่ 2 ตอนมี 100 → ถูกปฏิเสธ เพราะวันที่ 5 จะเหลือ −20) ถ้าเช็คแค่แถวตัวเองจะปล่อยผ่านแล้วพัง chain ทั้งเส้น
- **ทำไมแท็บ "รายวัน" ต้อง derive จาก `allMovements` ที่โหลดอยู่แล้ว ไม่ query ใหม่**: ledger ทั้งก้อนถูกดึงมาให้รายเดือนใช้อยู่แล้ว (`loadAllMovements`) การกรองตามช่วงวันเป็นแค่การ filter ในหน่วยความจำ → **สลับแท็บไม่มีค่าใช้จ่าย ไม่มีสถานะโหลด** และที่สำคัญกว่า: **ตัวเลขทุกแท็บมาจากแหล่งเดียวกัน** ถ้าแยก query จะมีโอกาสที่รายวันกับรายเดือนไม่ตรงกันเพราะดึงคนละเวลา. ค่าที่จ่ายคือต้อง `select` คอลัมน์เพิ่ม (`created_by,doc_no,supplier,note`) ซึ่งถูกกว่าการยิง query ที่สองมาก
- **ทำไมรายวันต้องคิด delta จาก balance chain ซ้ำอีกครั้ง แทนที่จะอ่าน `qty`**: เหตุผลเดียวกับ BUG34 — `adjust` เก็บยอดสัมบูรณ์ และแถวยกเลิกเป็น movement ตรงข้าม ถ้าอ่าน `qty` ตรง ๆ การนับสต๊อกจะโผล่มาเป็น "รับเข้า 140" ในบันทึกรายวัน. ใช้ `bucketTypeOf`-แบบเดียวกับรายเดือน (`dailyKindOf`) เพื่อให้ **ตัวเลขบนหน้ารายวันกับหน้ารายเดือนเล่าเรื่องเดียวกันเสมอ**
- **ทำไมสรุปรายวัน/รายช่วงรวมเป็น "บาท" ไม่รวมเป็น "จำนวน"**: 15 รายการมีหน่วยต่างกัน (กก./ลิตร/แผ่น) บวกกันแล้วไม่มีความหมาย — ตัวเลขเดียวที่บวกข้ามวัสดุได้คือมูลค่า. ส่วนจำนวนแสดงเป็น "กี่รายการ" ซึ่งบวกได้จริง
- **ทำไมประวัติต้องแบ่งหน้า ไม่ใช่แค่ขยาย limit**: `.limit(20)` เดิมทำให้ **รายการที่เก่ากว่านั้นกดยกเลิกไม่ได้ตลอดกาล** (ปุ่มยกเลิกอยู่บนแถว) การขยายเป็น 100 ก็แค่เลื่อนปัญหา. `.range(from, from+20)` ดึงเกินมา 1 แถวเพื่อ**ถามว่ายังมีต่อไหม** โดยไม่ต้อง `count` แยก — เห็นปุ่ม "ดูเพิ่ม" ก็แปลว่ายังมีจริง ๆ ไม่ใช่เดา
- **ทำไมเปลี่ยนจาก "3 วันก่อน" เป็นวันที่จริงในประวัติ**: relative time อ่านง่ายตอนดูว่า "เพิ่งเกิดอะไร" แต่งานจริงของหน้านี้คือ**สืบว่าวันที่ 5 เกิดอะไร** ซึ่ง relative ตอบไม่ได้เลย เก็บ relative ไว้เฉพาะภายใน 1 ชม. ที่มันชนะจริง
...
- **ทำไมแยก `viewStock` ออกจาก `viewERP`** (toun สั่ง 2026-09-25 — **แทนที่ข้อถัดไป**): toun ต้องการให้หน้า ERP เป็นของ**ระดับหัวหน้า** (admin/supervisor/office) ส่วนพนักงานหน้างานบันทึกสต๊อกได้โดย**ไม่ต้องเห็น ERP**. ธงเดียวทำสองอย่างนี้พร้อมกันไม่ได้ — ที่ผ่านมา staff ได้ `stockIn/stockOut` แล้วแต่หน้าสต๊อกปฏิเสธเพราะเช็ค `viewERP` **สิทธิ์ที่เปิดให้จึงใช้ไม่ได้จริงเลย** (เจอจากการทดสอบล็อกอินเป็น staff). ตอนนี้: `stock.html` เช็ค `viewStock` (`enforceStockAccess`) · `viewERP` ใช้ตัดสินแค่**ทางกลับ** (← ไป ERP หรือ PO) และหัวเล็ก ("ERP · คลังสต๊อก" / "PO Tracker · คลังสต๊อก") · หน้า PO โชว์ปุ่มสต๊อกเฉพาะ `viewStock && !viewERP` — ฝั่งออฟฟิศเข้าผ่าน ERP อยู่แล้ว **ได้ปุ่มเดียว ไม่ใช่สอง**. manager ถูกย้ายไปฝั่ง "เข้าตรง" ด้วย เพราะ toun ระบุชื่อแค่ 3 role — เปิดกลับได้ด้วยสวิตช์เดียวในแท็บ 🔐 ผู้ดูแล (เพิ่ม 2 แถว "เข้าคลังสต๊อก"/"เข้าหน้า ERP" ไว้ที่นั่นแล้ว). **ไม่ต้อง migrate schema** — `role_permissions` ไม่มี CHECK บน `permission_key`
- **ทำไมตัวเลขเงินใช้สวิตช์ `viewStockDocs` ตัวเดียว ไม่แยกธงใหม่** (toun สั่ง "ซ่อนตัวเลขจากพนักงาน" 2026-09-25): ธงนี้ซ่อนราคาซื้อ/ผู้ขาย/เลขใบส่งของอยู่แล้วในรายวัน·Excel·CSV·พิมพ์ แต่**มูลค่าคลัง · คอลัมน์ราคา/มูลค่า · ยอดรวมหมวด · แท็บรายเดือน · ราคาเฉลี่ยตอนรับเข้า** ยังโชว์ให้ทุกคน = ซ่อนครึ่งเดียว. ถ้าแยกเป็นสองธง จะมีสถานะแปลก ๆ อย่าง "เห็นมูลค่าคลังแต่ไม่เห็นราคา" ที่ไม่มีใครต้องการ → รวมเป็นสวิตช์เดียว `canSeeMoney()` **เห็นเงินหรือเห็นแค่จำนวน ไม่มีครึ่ง ๆ**. ค่าจริงในฐานตรงกับที่ toun ต้องการอยู่แล้ว (เปิด: admin/supervisor/office · ปิด: manager/staff) **ไม่ต้องแก้ DB**. ช่องราคาตอนรับเข้า**ไม่ซ่อน**แต่เริ่มว่าง (เดิมเติมราคาเฉลี่ยให้ = เผยตัวเลข) และตัดข้อความเทียบกับค่าเฉลี่ยทิ้ง — เว้นว่างแล้ว server ใช้ `COALESCE(p_unit_cost, avg)` ค่าเฉลี่ยไม่เพี้ยน. PC ตารางซ่อน 2 คอลัมน์ผ่าน `body.no-money .col-money` เพราะ `<th>` เป็น HTML คงที่. ทดสอบ 4 role × มือถือ/PC สแกนหา "บาท" หรือเลขมีคอมม่าทุกหน้า 72/72
- **ทำไมรวมเฉดด้วย `report_group` แบบข้อความ ไม่ใช่ parent_id** (toun ขอ 2026-09-25 — "สีจริงมีหลายเฉด 801, 802 แต่ส่งสำนักงานใส่แค่สีจริง"): แต่ละเฉดต้องเป็นแถวของตัวเอง เพราะ**ยอดและต้นทุนเฉลี่ยต่างกันจริง** (801 ราคาไม่เท่า 802) และการเบิกต้องรู้ว่าหยิบเฉดไหน — รวมเป็นแถวเดียวไม่ได้. ส่วน**การรวม**เป็นเรื่องของการแสดงผลกับรายงานล้วน ๆ จึงไม่แตะ ledger เลย (`apply_stock_movement` ไม่รู้จักกลุ่ม). เลือก**ข้อความ + กุญแจ = report_group หรือชื่อตัวเอง** แทน FK เพราะ: (1) วัสดุ "สีจริง" เดิม 18 ลิตรที่มี ledger แล้ว**กลายเป็นหัวกลุ่มได้ทันที ไม่ต้องย้ายข้อมูล** (2) ถ้าปิดตัวเดิมทิ้งตอนตั้งต้นใหม่ กลุ่มยังอยู่ ไม่ต้องมี "แถวหัวกลุ่มปลอม" ที่ไม่มีของจริง (3) ถ้าใช้ parent_id หัวกลุ่มจะเป็นทั้ง "วัสดุ" และ "หมวด" ในแถวเดียว ต้องตัดสินว่ายอดของมันนับหรือไม่นับ. ค่าที่จ่าย: เปลี่ยนชื่อหัวกลุ่มต้องไล่อัปเดตเฉดตาม (`saveStock` ทำให้แล้ว) · **กติกาที่บังคับ**: หน่วยทุกเฉดต้องตรงกัน (บวกลิตรกับถังไม่ได้ และฟอร์มกระทรวงรับหน่วยเดียว) — ถ้าข้อมูลเก่าหน่วยไม่ตรง หัวกลุ่มโชว์ "หน่วยไม่ตรงกัน" แทนการบวกผิด ๆ · เข้ากลุ่ม = รับหมวด + `ministry_seq` ของกลุ่มอัตโนมัติ. **ยอดรวมบนหัวกลุ่มนับทุกเฉดเสมอ** แม้ค้นหาจนเหลือเฉดเดียว (บอก "แสดง 1 จาก 3") เพราะคำถามของหัวกลุ่มคือ "สีจริงมีทั้งหมดเท่าไหร่" · รายงานกระทรวง: รวมจำนวนทุกช่อง ราคา = ยอดเงินซื้อรวม ÷ จำนวนซื้อรวม (เฉลี่ยถ่วงน้ำหนัก) · **กับดักที่เจอตอนทำ**: `renderStockTable` มีตัวแปร local ชื่อ `groupTotals` อยู่แล้ว ฟังก์ชันใหม่ชื่อเดียวกันถูกบังจนตาราง PC ไม่มีแถวรวม — ตั้งชื่อใหม่เป็น `groupSum` · ทดสอบ 27/27
- **ทำไมหน่วยวัสดุเป็นรายการที่ต้องเพิ่มผ่านปุ่ม ไม่ใช่ช่องพิมพ์อิสระ** (toun สั่ง 2026-09-26): ช่องพิมพ์อิสระทำให้หน่วยเดียวกันมีหลายตัวสะกด ("กก." / "กิโลกรัม" / "โล") ซึ่งตอนนี้**มีผลจริงแล้ว** — กลุ่มรวมยอด (`report_group`) บังคับหน่วยตรงกันเป๊ะ และรายงานกระทรวงรับหน่วยเดียวต่อบรรทัด. เก็บใน `config` type `stock_unit` (ที่เดียวกับ master list อื่น — ไม่สร้างตารางใหม่) **ต้องขยาย CHECK `config_config_type_check` ก่อน** ไม่งั้น insert ล้มเงียบ (ซ้ำรอย BUG17/19/31 เป็นรอบที่ 4 ถ้าลืม). ปุ่ม "＋ เพิ่มหน่วย" คุมด้วยธงใหม่ `stockAddUnit` (admin/supervisor/office เปิด · manager/staff ปิด · สลับได้ในแท็บ 🔐 ผู้ดูแล) กั้น 2 ชั้น: ซ่อนปุ่ม + `addStockUnit()` ปฏิเสธเองถ้าไม่มีสิทธิ์. คนที่เพิ่มวัสดุได้แต่ไม่มีธงนี้ (manager) **ยังเลือกจากรายการได้** แค่เพิ่มหน่วยใหม่ไม่ได้. รายการที่โชว์ = config + หน่วยที่วัสดุใช้อยู่จริง → หน่วยเก่าที่ไม่อยู่ในรายการยังเลือกกลับได้ตอนแก้ไข ไม่ถูกเปลี่ยนเป็นตัวแรกเงียบ ๆ · พิมพ์ซ้ำ (ต่างแค่ตัวพิมพ์/ช่องว่าง) = เลือกตัวเดิมให้ ไม่เพิ่มซ้ำ · ~~ยังไม่มีปุ่มลบหน่วย~~ (มีแล้ว — ดูข้อถัดไป) — หน้า PO อ่าน config แบบกรองตาม type อยู่แล้ว แถว `stock_unit` ไม่รั่วไปที่นั่น · ทดสอบ 21/21
- **ทำไมลบหน่วยต้อง "ย้ายวัสดุไปหน่วยอื่น" ก่อนเสมอ และลบแบบ hard delete** (toun สั่ง 2026-09-26 — "ก่อนลบให้เตือน บอกว่าวัสดุไหนใช้หน่วยนั้น แล้วจะเปลี่ยนเป็นหน่วยไหนแทน"): หน่วยที่หายไปจากรายการแต่ยังติดอยู่บนวัสดุ = ข้อมูลที่เลือกกลับไม่ได้และรวมกลุ่มไม่ได้ จึง**ไม่มีทางลบโดยทิ้งวัสดุไว้กับหน่วยเดิม**. ใช้ธง `stockAddUnit` ตัวเดิม (ป้ายเปลี่ยนเป็น "เพิ่ม/ลบหน่วย") — คนที่ดูแลรายการคือคนเดียวกัน ไม่แยกธงให้สับสน · กั้น 2 ชั้นเหมือนปุ่มเพิ่ม. **ขั้นตอน**: แผงเตือน query `stock_items` **สดจาก DB ทุกสถานะ** (ไม่ใช่ `stockItems` ที่มีแค่ตัวที่เปิดใช้ — ตัวที่ปิดไปแล้วจะค้างหน่วยที่ไม่มีอยู่) → แสดงชื่อ + ยอด "80 กิโลกรัม → 80 กก." + นับตัวที่ปิดใช้งาน + **บอกตรง ๆ ว่าตัวเลขไม่ถูกแปลง** (ลิตร→ถัง ต้องปรับยอดเอง) + เตือนถ้าการย้ายทำให้กลุ่มรวมยอดหน่วยไม่ตรงกัน → บังคับเลือกหน่วยแทนถ้ามีคนใช้ → `confirm()` → **UPDATE วัสดุก่อน แล้วค่อย DELETE แถว config** (ถ้า update พัง ไม่มีอะไรหาย · ถ้า delete พังหลัง update ก็แค่มีหน่วยว่าง ๆ ค้างในรายการ). UPDATE ใช้ `.eq('unit', X)` ณ ตอนกด ไม่ใช่รายชื่อที่โชว์ → วัสดุที่เพิ่งเพิ่มระหว่างเปิดแผงก็ถูกย้ายด้วย · ถ้าแผงบอก "ไม่มีใครใช้" แต่ตอนกดมีคนเพิ่งใช้ → หยุด แจ้ง แล้วตรวจใหม่. **hard delete ไม่ใช่ `is_active=false`** เพราะไม่มีอะไรอ้างถึงแถว config (วัสดุเก็บหน่วยเป็นข้อความ) และถ้าปิดแบบ soft แล้วมาเพิ่มชื่อเดิมอีกจะได้แถวซ้ำ · ห้ามลบหน่วยสุดท้าย (ไม่งั้น `loadStockUnits` ตกไปใช้ FALLBACK) · ไม่ต้อง migrate — RLS `config`/`stock_items` เปิด delete/update อยู่แล้ว · log `stock_unit_delete` {unit, replaced_with, items} · ทดสอบ 31/31 + ชุดเดิม 157/157
- ~~**ทำไม `viewERP` ครอบ `stock.html` ด้วย**~~ (เลิกใช้ 2026-09-25 — ดูข้อบน): stock เป็นโมดูลใต้ ERP เข้าถึงได้จากหน้า ERP เท่านั้น และแสดง**ต้นทุนกับราคาผู้ขาย**ซึ่งเป็นข้อมูลเชิงพาณิชย์ สิทธิ์ `stockIn/Out/...` เดิมคุมแค่ "แก้ได้ไหม" ไม่ได้คุม "เห็นได้ไหม" — ใช้ธงเดียวกันทั้งสองหน้าจึงตรงกับที่ toun สั่งว่า "เข้าหน้า ERP ได้เฉพาะคนที่มีสิทธิ์"
...
- **ทำไม "ตั้งค่าผู้ใช้งาน" ใน ERP เป็น admin เท่านั้น ไม่ใช่ `manageUsers`** (toun สั่ง 2026-08-25): หน้านี้ **แสดงรายชื่อทุกคนพร้อมความยาวรหัสของแต่ละคน** ซึ่งเป็นข้อมูลที่ช่วยเดารหัสได้ (รู้ว่าใครใช้ 4 หลัก = เดา 10,000 แบบ ไม่ใช่ 1,000,000) และเปลี่ยนรหัสคนอื่นได้ = **สวมสิทธิ์เป็นคนนั้นได้ทันที** ธงนี้จึงกว้างเกินไปเมื่อ `manageUsers` เปิดให้ supervisor/office ด้วย. ใช้ `currentUser.role === 'admin'` ตรง ๆ เหมือนแท็บ 🔐 สิทธิ์ ใน stock.html. กั้น **3 ชั้น** ตามบทเรียน `enforceERPAccess`: ซ่อนการ์ด + ซ่อนเมนูแถบซ้าย (`.role-admin-only`) · `openUserSettings()` ปฏิเสธ + เขียน `user_settings_denied` ลง activity_log · `openPinModal()` / `savePin()` ปฏิเสธซ้ำ — **ซ่อนปุ่มไม่ใช่การล็อกประตู**
- **ผลที่ยอมแลก**: คนอื่นเปลี่ยนรหัสตัวเองไม่ได้แล้ว ต้องให้ admin ตั้งให้ทุกคน. เดิมเปิดให้เปลี่ยนของตัวเองเพราะ "รหัสที่เปลี่ยนไม่ได้คือรหัสที่สุดท้ายจะถูกบอกต่อ" แต่ toun สั่งชัดว่าคนอื่น **เห็นไม่ได้เลย** — และโรงงานนี้มีผู้ใช้ 10 คน admin ตั้งให้เองไหว ถ้าอนาคตคนเยอะขึ้นค่อยทำหน้า "เปลี่ยนรหัสของฉัน" แยกที่ไม่โชว์รายชื่อคนอื่น
- **ทำไมปุ่ม 👁 ดูรหัสจริงถึงใส่ได้ ทั้งที่เพิ่งบอกว่ารายชื่อ+ความยาวรหัสเป็นข้อมูลอ่อนไหว** (toun ขอ 2026-08-25): เหตุผลที่เคยกั้นคือ **ใครเห็น** ไม่ใช่ **เห็นอะไร** — พอหน้านี้เป็น admin เท่านั้นแล้ว คนที่อ่านได้ก็คือคนที่ตั้งรหัสให้ทุกคนอยู่แล้ว ปุ่มดูรหัสจึงไม่เพิ่มอำนาจอะไรใหม่ แต่ช่วยตอนบอกรหัสให้พนักงานทางโทรศัพท์. ป้องกันเท่าที่ควร: ดีฟอลต์ปิดหมด · เปิดทีละแถว · **กดปิดหน้าต่างแล้วล้าง `usShown` ทิ้ง** เพื่อให้คนที่เปิดหน้านี้ครั้งถัดไปต้องกดขอเองใหม่ ไม่ใช่รับมรดกจอที่เปิดค้างไว้
- **ทำไมหัวข้อหมวดใช้โครงเดียวกับ `.cat-section-head` ใน stock.html**: เป็นรายการที่แบ่งกลุ่มเหมือนกัน (หมวดวัสดุ ↔ ตำแหน่ง) — ก๊อป token เดิมทั้งชุด (13px/800/`--ink-2` + meta 11.5px/`--ink-3` + margin `16px 2px 8px`) แทนที่จะคิดขนาดใหม่ ตามกฎ design consistency. เรียงจากอำนาจมากไปน้อย (admin → หัวหน้า → office → ผู้จัดการ → พนักงาน) และ role ที่ไม่รู้จักตกไปท้ายสุด **ไม่หายไปเฉย ๆ**
- **ทำไมแถวต้องพับบรรทัดบนจอแคบ**: ชื่อ + รหัส + 👁 + ปุ่มเปลี่ยนรหัส เรียงแถวเดียวกินความกว้างคงที่ ~230px บน iPhone SE (320px) เหลือให้ชื่อไม่ถึง 50px — ชื่อยาวอย่าง "พนักงานส่ง/รับของ น้ำเป็ล" จะบี้จนอ่านไม่ออก. `flex-wrap` + `.us-who { flex: 1 1 100% }` ที่ ≤430px ดันชื่อขึ้นบรรทัดของตัวเอง
- **บทเรียนเรื่องการตั้งชื่อปุ่ม: "ตั้งต้นใหม่" ถูกอ่านว่า "ล้างก่อน แล้วค่อยกรอกทีหลัง"** (2026-09-25): toun เข้าใจลำดับผิดโดยธรรมชาติ — คิดว่าต้องล้างให้ว่างก่อน แล้วไปเดินนับ แล้วเอาตัวเลขมาใส่ช่อง "นับสต๊อก" ทั้งที่หน้าตั้งต้น**ขอยอดที่นับได้ตั้งแต่ตอนกดอยู่แล้ว**. ไม่ใช่ความผิดของ toun — **คำว่า "reset/ล้าง" สื่อถึงการทำให้ว่าง ไม่ได้สื่อว่ามันขออะไรกลับ** พอปุ่มหนึ่งทำสองอย่างในจังหวะเดียว (ล้าง + ตั้งค่าใหม่) ชื่อที่เน้นครึ่งแรกจะทำให้คนวางแผนงานผิดลำดับ. **สิ่งที่ code ทำไว้ถูกแล้ว** (กรอกก่อนล้างในหน้าจอเดียว ปลอดภัยกว่า) แต่ถ้าเจอคนเข้าใจผิดซ้ำอีก ให้แก้ที่**ข้อความบนหน้าจอ** — เขียนหัวข้อทำนอง "กรอกยอดที่นับได้ → ระบบจะล้างของเก่าแล้วเริ่มจากตัวเลขนี้" ไม่ใช่แก้ลำดับการทำงาน. บทเรียนทั่วไป: **ถ้าผู้ใช้อธิบายลำดับการใช้งานกลับด้านกับที่เราออกแบบ แปลว่าชื่อหรือคำอธิบายบนจอสื่อผิด ไม่ใช่ผู้ใช้อ่านไม่ละเอียด**
- **ทำไมต้องมีทั้ง "นับสต๊อก" และ "ตั้งต้นใหม่" ไม่ใช่เลือกอย่างใดอย่างหนึ่ง** (toun ถาม 2026-09-09): เป็นคนละปัญหากันคนละเรื่อง. **นับสต๊อก** = ปรับยอดให้ตรงของจริง **เก็บประวัติไว้** → เขียน `adjust` ทีละแถว รายงานเดือนที่ปิดไปแล้วไม่ขยับ · เป็นงานประจำที่ทำได้ตลอดไป. **ตั้งต้นใหม่** = ล้าง ledger แล้วเริ่มนับ 1 → ถูกต้องแค่**ครั้งเดียว** ตอนนี้ ขณะที่ข้อมูลยังเป็นของที่กรอกไว้ตอนสร้างระบบ (ตรวจฐานจริง 2026-09-09: **วัสดุปลอม 6 รายการ** ที่ไม่มี `ministry_seq` · รับเข้าที่เลขใบส่งของเป็น "2" · เบิกสีพื้น 111 ลิตรที่ไม่เคยเกิด · และ **5 รายการมี qty แต่ไม่มี ledger รองรับเลย**) ถ้าเอาไปใช้หลังเริ่มเก็บจริง = ทำลายหลักฐานที่รายงานกระทรวงยืนอยู่บน
- **ทำไมกันปุ่มล้างด้วย "พิมพ์คำ" ไม่ใช่แค่ confirm()**: กล่อง confirm กดผ่านได้ด้วยนิ้วเดียวโดยไม่ต้องอ่าน — เหมือนที่ BUG41 สรุปไว้ตอนยอดติดลบ. บังคับพิมพ์ **"ล้างข้อมูล"** ทำให้ต้องใช้สมองอีกชั้น และปุ่มจะกลับมาล็อกเองทุกครั้งที่ออกจากแท็บแล้วกลับมา (ไม่ให้ค้างติดอาวุธไว้ให้คนถัดไป). ซ้อนอีก 3 ชั้น: `role === 'admin'` ตรง ๆ **โดยตั้งใจไม่ทำเป็น permission flag** เพราะ flag มีไว้แจก · server ปฏิเสธถ้าไม่ส่ง `p_confirm='RESET-STOCK'` · และ**ดาวน์โหลด Excel สำรองทั้ง ledger ให้ก่อน ถ้าสำรองไม่สำเร็จจะไม่ล้าง** — เปลี่ยนงานที่ย้อนไม่ได้ ให้เป็นแค่งานที่กู้คืนลำบาก
- **ทำไมแยก `movementRows(from,to)` ออกจาก `dailyRows()`**: ไฟล์สำรองก่อนล้างต้องเป็น ledger ทั้งก้อน แต่ `xlRowFromMovement()` กินแถวรูปแบบ**หน้าจอ** (`{mv, day, delta, kind, name, unit}`) ไม่ใช่แถวดิบจาก DB — ตอนแรกผมส่ง `allMovements` ตรง ๆ ไฟล์สำรองเลยพังเงียบ ๆ แล้ว**การล้างก็ถูกยกเลิกตามที่ออกแบบไว้** (เจอจากเทสต์ ไม่ใช่จากอ่านโค้ด). แทนที่จะเขียนสูตร delta ซ้ำ ดึงตัวสร้างแถวออกมาเป็นฟังก์ชันกลาง — รายวัน · Excel export · ไฟล์สำรอง ใช้โค้ดชุดเดียวกัน ไม่มีทาง drift
- **6 หลักเท่านั้น — เลิกรองรับ 4 หลักถาวร** (toun สั่ง 2026-09-09): ระบบผสม 4/6 หลักมีไว้เพื่อ**ช่วงเปลี่ยนผ่าน**เท่านั้น พอ toun ตัดสินใจย้ายทุกคนเป็น 6 หลัก กลไกที่ค้ำระบบผสมก็หมดหน้าที่และถูกลบทิ้งทั้งชุด: `PIN_MIN/PIN_MAX` → `PIN_LEN = 6` · ตัวลองล็อกอินที่หลักที่ 4 (`tentative`) · ตัวจับเวลา 1.5 วิ (`pinTentativeMiss`) · กฎห้ามรหัสขึ้นต้นเหมือนกันใน `pinClash`. **เหตุผลที่กฎ prefix ไปได้ ไม่ใช่เพราะเลิกอันตราย แต่เพราะสิ่งที่มันกันหายไป** — แป้นไม่ยิงที่หลักที่ 4 อีกแล้ว รหัส 4 หลักที่ค้างอยู่จึงไม่มี "4 ตัวแรก" ให้ไปบังของใคร ถ้ายังเก็บกฎไว้จะกลายเป็นตัวปฏิเสธรหัส 6 หลักที่ถูกต้องโดยไม่มีเหตุผล. เหลือกฎเดียวที่ยังจำเป็น: **ห้ามรหัสซ้ำกันตรง ๆ** เพราะ login หาคนจากรหัสอย่างเดียว
- **ผลที่ยอมแลก + วิธีที่เลือกรับมือ**: ตอน deploy มีพนักงาน **5 คนยังใช้รหัส 4 หลัก** (delivery/production/qc1/qc2/qc3) — **เข้าระบบไม่ได้ทันทีจนกว่า admin จะตั้งรหัสใหม่ให้**. ไม่ปล่อยให้ admin ไปไล่หาเอง: หน้าตั้งค่าผู้ใช้งานนับให้เลยว่าเหลือกี่คน + ตีกรอบแดงเฉพาะแถวนั้น + เขียนในแถวว่า "เข้าระบบไม่ได้ ต้องตั้งรหัสใหม่" (`.us-row.is-stale`). **หลักการ: ถ้า deploy แล้วมีคนใช้งานไม่ได้ หน้าจอที่แก้ปัญหานั้นได้ต้องบอกเองว่าใครบ้าง ไม่ใช่ให้ไปอ่านใน chat**
- **ทำไมแป้น PIN ต้องโชว์ครบ 6 จุดตั้งแต่แรก ไม่ใช่โผล่เพิ่มตอนพิมพ์หลักที่ 5** (toun สั่ง 2026-09-09): ตอนทำระบบผสม 4/6 หลัก ผมให้จุดที่ 5-6 ซ่อนไว้ก่อนแล้วโผล่มาเมื่อพิมพ์เกิน 4 — คิดว่าเป็นการ "ไม่หลอกคนที่ใช้ 4 หลักว่าต้องพิมพ์ 6" แต่ผลจริงตรงกันข้าม: **หน้าจอบอกว่า "ใส่ PIN 4 หลัก" พร้อมจุด 4 จุด คนที่ถือรหัส 6 หลักจะคิดว่าตัวเองจำผิดหรือระบบยังไม่อัปเดต** และการมีของโผล่มากลางคันทำให้ layout ขยับใต้นิ้ว. โชว์ครบ 6 ตั้งแต่แรก = แป้นบอกความจริงว่ารับได้ถึง 6 หลัก ส่วนคนที่ใช้ 4 หลักยังเข้าได้ทันทีที่หลักที่ 4 เหมือนเดิม (ตรรกะ `PIN_MIN`/`PIN_MAX` ไม่แตะเลย เปลี่ยนแค่การแสดงผล) · ข้อความใต้จุดแก้เป็น "4 หรือ 6 หลัก" ทั้ง 4 ไฟล์
- **ทำไมแถบเมนูซ้ายต้องตัดสินใจเองตอน render ไม่รอ `applyPermissions`**: `renderDkNav()` วาดใหม่ทุกครั้งที่ข้อมูลรีเฟรช ถ้าปล่อยให้ปุ่มโผล่มาก่อนแล้วรอ `applyPermissions` มาซ่อน จะ**เห็นแวบ ๆ ทุกครั้งที่รีเฟรช** — ใส่ `style="display:none"` ตั้งแต่ตอนสร้าง string แล้วยังติดคลาส `.role-admin-only` ไว้เป็นชั้นสอง
