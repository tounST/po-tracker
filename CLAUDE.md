...
- **Cache version ปัจจุบัน**: `po-tracker-v51` — bump ทุกครั้งที่ modify cached files (ตอนนี้ cache: index / po-mobile / po-desktop / **erp** / **stock**)
...
| **ให้พนักงาน (staff) เปิดหน้าสต๊อกได้** | ⚠️ **รอ toun เลือก** — `stockIn`/`stockOut` เปิดแล้ว **แต่ `viewERP`=false** → `stock.html` ขึ้นหน้าปฏิเสธ ใช้ไม่ได้ทั้ง 5 คน · ทางเลือก: เปิด `viewERP` ให้ staff (แตะสวิตช์) หรือแยกธง `viewStock` (แก้ code) · ดู `docs/STATUS.md` ลำดับ 3 |
